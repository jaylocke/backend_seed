...
gulp.task('scripts:app', function() {
    ...
    return gulp.src(source.scripts)
        ...
        .pipe( $.if(isProduction, gulp.dest(paths.rev + 'app') ))
        ;
});
...
gulp.task('vendor:base', function() {
    ...
    return gulp.src(vendor.base.source)
        ...
        .pipe( $.if(isProduction, rev.manifest() ))
        .pipe( $.if(isProduction, gulp.dest(paths.rev + 'base') ))
        ;
});
...
gulp.task('styles:app', function() {
    ...
    return gulp.src(source.styles.app)
        ...
        .pipe( $.if(isProduction, rev.manifest() ))
        .pipe( $.if(isProduction, gulp.dest(paths.rev + 'style') ))
        ;
});
...
//添加MD5戳
gulp.task('rev-collector',gulpsync.sync(['rev-index']) );

gulp.task('rev-index', function () {
    return gulp.src([paths.rev + '**/*.json', build.templates.index + 'index.html'])
        .pipe( revCollector() )
        .pipe( gulp.dest(build.templates.index) );
});
